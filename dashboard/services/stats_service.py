...
def get_medicine_stats():
    ...
    try:
        rows = db.execute("""
            SELECT
                COALESCE(
                    NULLIF(TRIM(detection_record.medicine_category), ''),
...
def get_latest_detections(limit=20):
    ...
    rows = get_db().execute(
        "SELECT id, medicine_name, medicine_category, medicine_efficacy, "
        "confidence, emotion, emotion_confidence, created_at "
        "FROM detection_record "
