...
def _migrate_schema(conn):
    ...
    detection_columns = {
        row[1] for row in conn.execute("PRAGMA table_info(detection_record)").fetchall()
    }
    if "medicine_category" not in detection_columns:
        conn.execute(
            "ALTER TABLE detection_record ADD COLUMN medicine_category TEXT DEFAULT ''"
        )
    if "medicine_efficacy" not in detection_columns:
        conn.execute(
            "ALTER TABLE detection_record ADD COLUMN medicine_efficacy TEXT DEFAULT ''"
        )
