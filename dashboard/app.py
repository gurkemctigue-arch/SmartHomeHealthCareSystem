...
def create_app(test_config=None):
    ...
    def recognition_status():
        """返回药盒定位、OCR 与药品库运行状态。"""
        try:
            from services.video_service import detector as current_detector

            data = current_detector.status()
        except Exception as exc:
            logger.exception("读取药品识别状态失败")
            data = {"available": False, "error": str(exc)}
        return jsonify({"code": 200, "message": "ok", "data": data})

    @app.route("/api/recognition/current")
    def current_recognition():
        """返回与摄像头视频流同步的结构化识别结果。"""
        return jsonify({
            "code": 200,
            "message": "ok",
            "data": get_latest_recognition(),
        })
    ...
    def health():
        ...
        try:
            ...
            status["detector"] = "YOLO-OCR" if _det.is_real_model else "unavailable"
            status["medicine_recognition"] = _det.status()
        except Exception:
            status["detector"] = "unavailable"
            status["medicine_recognition"] = {"available": False}
